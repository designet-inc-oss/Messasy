 ...
 *
 * $RCSfile: libdgmail.c,v $
 * $Revision: 1.3 $
 * $Date: 2010/03/09 04:29:16 $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
    ...
    while (*ptr != '\0') {
        if (*ptr == '=' && *(ptr + 1) == '?') {
            ...
            if (!strncasecmp(subptr, MIME_JISB_STR, 14) &&
                ...
                bqflag = BQ_QP;
                codeflag = CODE_EUC;
                /* Decode MIME (UTF8+BASE64) */
            } else if (!strncasecmp(subptr, MIME_UTF8B_STR, 8) &&
                       (mimeend = strstr(subptr + 8, "?="))) {
                skiplen = 8;
                bqflag = BQ_B64;
                codeflag = CODE_UTF8;
                /* Decode MIME (UTF8+QP) */
            } else if (!strncasecmp(subptr, MIME_UTF8Q_STR, 8) &&
                       (mimeend = strstr(subptr + 8, "?="))) {
                skiplen = 8;
                bqflag = BQ_QP;
                codeflag = CODE_UTF8;
            ...
            *mimeend = '\0';
            ...
            if (!check_7bit(decbuf)) {
                ...
                case CODE_UTF8:
                    ret = utf82jis_iconv(decbuf, &trbuf);
                    break;
                }
            }
