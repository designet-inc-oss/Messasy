...
struct cfentry maildrop_cfe[] = {
    {
        ...
        MESSASY_OFFSET(struct maildrop_config, cf_maildir), is_writable_directory
    },
    {
        "MailFolder", CF_STRING, NULL,
        MESSASY_OFFSET(struct maildrop_config, cf_mailfolder), is_mailfolder
    },
    {
        "DotDelimiter", CF_STRING, ",",
        MESSASY_OFFSET(struct maildrop_config, cf_dotdelimiter), is_dotdelimiter
    },
    {
        "SlashDelimiter", CF_STRING, "_",
        MESSASY_OFFSET(struct maildrop_config, cf_slashdelimiter), is_slashdelimiter
    }
};
 ...
 *
 * 機能:
 *    maildropモジュール用のextra configの作成
 *
 * 引数:
 *    char *modname                     モジュール名
